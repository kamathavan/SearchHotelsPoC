...
import com.app.test.search.place.ui.theme.color_blue
import com.app.test.search.place.ui.theme.color_cyan
...
fun SearchInputField(
    ...
    if (searchFieldState is SearchFieldState.WithInputActive && isKeyboardOpen) {
        onKeyboardHidden()
    }
    ...
    TextField(
        ...
        leadingIcon = {
            when (searchFieldState) {
                ...
                SearchFieldState.WithInputActive -> MyIcon(
        ...
        colors = when (searchFieldState) {
            SearchFieldState.Idle -> searchFieldColorsStateIdle()
            SearchFieldState.EmptyActive,
            SearchFieldState.WithInputActive -> searchFieldColorsStateActive()
        },
        trailingIcon = if (searchFieldState is SearchFieldState.WithInputActive) {
            {
                MyIcon(
            ...
            MyText(
                ...
                style = TextStyle(
                    ...
                    color = when (searchFieldState) {
                        SearchFieldState.EmptyActive -> color_silver.copy(alpha = 0.6f)
                        SearchFieldState.Idle -> color_blue.copy(alpha = 0.6f)
                        is SearchFieldState.WithInputActive -> color_silver.copy(
                            alpha = 0.6f
                        )
                    },
                )
